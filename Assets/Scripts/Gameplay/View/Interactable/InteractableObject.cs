...
namespace RogueIslands.Gameplay.View
{
    public class InteractableObject : MonoBehaviour
    {
        ...
        [SerializeField] private float _dragThreshold = 5;
        ...
        /// <summary> An event that occurs to a InteractableObject. </summary>
        [System.Serializable]
        public class InteractableEvent : UnityEvent<InteractableObject>
        {
        }
        ...
        private static List<InteractableObject> _hoveredObjects = new List<InteractableObject>();

        /// <summary> The currently hovered objects. </summary>
        public static List<InteractableObject> HoveredObjects => _hoveredObjects;

        /// <summary> The first hovered object. </summary>
        public static InteractableObject HoveredObject => _hoveredObjects.Count > 0 ? _hoveredObjects[0] : null;

        private static List<InteractableObject> _selectedObjects = new List<InteractableObject>();

        /// <summary> The currently selected / dragged objects. </summary>
        public static List<InteractableObject> SelectedObjects => _selectedObjects;

        /// <summary> The first selected / dragged object. </summary>
        public static InteractableObject SelectedObject => _selectedObjects.Count > 0 ? _selectedObjects[0] : null;
        ...
        private static InteractableRaycaster _raycaster = new InteractableRaycaster();
        ...
        private InputProvider GetInputProvider()
        {
            ...
            {
                inputProvider = Flexalon.Flexalon.GetInputProvider();
            }
        ...
        private void UpdateState(InputMode inputMode, Ray ray, bool isHit, bool isActive, bool becameActive)
        {
            ...
            if (_state == InteractableState.Selecting)
            {
                if (!isActive)
                {
                    if (_clickable && isHit && (ray.origin - _selectPosition).magnitude <= _dragThreshold)
                    {
                        Clicked.Invoke(this);
                    }
                ...
                else if (_draggable && (!_clickable || (ray.origin - _selectPosition).magnitude > _dragThreshold))
                {
        ...
        private void OnSelectStart(Vector3 position)
        {
        ...
        private void OnDragStart(InputMode inputMode, Ray ray)
        {
            ...
            var placeholderObj = Flexalon.Flexalon.AddComponent<FlexalonObject>(_placeholder);
            _node = Flexalon.Flexalon.GetOrCreateNode(gameObject);
        ...
        private void UpdateTarget(Ray ray)
        {
            ...
            else
            {
                ...
                if (_placeholder.gameObject.activeSelf && Flexalon.Flexalon.GetOrCreateNode(_placeholder).HasResult)
                {
        ...
        {
            var node = Flexalon.Flexalon.GetNode(gameObject);
